...
class ParallelLMHead(VocabParallelEmbedding):
    ...
    def __init__(
        ...
        *,
        ...
        # Expose the LinearBase-shaped geometry that quantization schemes read off the
        # layer. ParallelLMHead is the only quantizable projection that does NOT derive
        # from LinearBase, so schemes such as CompressedTensorsW8A16Fp8 raise
        # AttributeError on it when a checkpoint quantizes lm_head. Every value below is
        # derived from state the layer already tracks; nothing changes for an
        # unquantized lm_head.
